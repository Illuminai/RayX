...
bool firstIntersectionWithSphere(struct ray_t* ray,
                                 __global struct shape_t* sphere,
                                 struct intersection_t* inter);
...
bool firstIntersectionWithSDF(struct ray_t* ray, __global struct shape_t* shape,
                              struct intersection_t* intersection);
...
float subtractionSDF(float3 point,
                     __global struct subtractionSDF_t* subtraction);
...
#define sdfNormal(POINT, SDFFUN, OBJ)                     \
    (normalize((float3){                                  \
        SDFFUN(POINT + (float3){EPSILON, 0, 0}, OBJ) -    \
            SDFFUN(POINT - (float3){EPSILON, 0, 0}, OBJ), \
        SDFFUN(POINT + (float3){0, EPSILON, 0}, OBJ) -    \
            SDFFUN(POINT - (float3){0, EPSILON, 0}, OBJ), \
        SDFFUN(POINT + (float3){0, 0, EPSILON}, OBJ) -    \
            SDFFUN(POINT - (float3){0, 0, EPSILON}, OBJ), \
    }))
