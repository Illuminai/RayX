...
__kernel void render(  __write_only image2d_t resultImage,
    ...
    struct ray_t rayToCheck = (struct ray_t) {cameraPosition.xyz,
        normalize(cameraPosition.xyz + viewDirection * cameraFOV +
...
//    }
    ...
    if(inter.ray == 0) {
        ...
        float alpha = dot(inter.normal, normalize((double3){1,1,-1}));
        if(alpha > 0) {
            alpha = 1;
        } else {
            alpha = 0;
        }
        color = (float4){1,1* alpha,1, alpha};
    }
